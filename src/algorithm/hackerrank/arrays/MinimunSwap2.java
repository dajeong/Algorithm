...
public class MinimunSwap2 {
    ...
    static int minimumSwaps(int[] arr) {
        ...
        for (int i = 0; i < length; i++) {
            sorted.add(new Entry(i, arr[i]));
        }
        sorted.sort(Comparator.comparingInt(Entry::getValue));

        int count = 0;
        for (int i = 0; i < length; i++) {
            int index = sorted.get(i).getIndex();
            ...
            while (!visited[nextIndex]) {
                ...
                nextIndex = sorted.get(nextIndex).getIndex();
            }
            count += cycleCount - 1;
        }
        return count;
